...
use CkoCheckoutPayment\Components\CheckoutApi\Request\SofortPaymentRequestService;
...
class SofortPaymentRequestServiceTest extends TestCase
{
    /**
    ...
    /**
     * @var SofortPaymentRequestService
     */
    private $sofortPaymentRequestService;
    ...
    protected function setUp(): void
    {
        ...
        $this->apiClientServiceMock = new CheckoutApiClientServiceMock(SofortPaymentMethod::NAME);
        $this->sofortPaymentRequestService = new SofortPaymentRequestService(
    ...
    public function testSendPaymentRequest(): void
    {
        ...
        $paymentResponse = $this->sofortPaymentRequestService->sendPaymentRequest($paymentRequestStruct);
    ...
    public function testSendPaymentRequestThrowsExceptionWhenApiExceptionIsThrown(): void
    {
        ...
        static::expectExceptionMessage('Checkout.com API Request has failed for the reason: '. SofortPaymentMock::class);

        $this->apiClientServiceMock->setShouldThrowApiException(true);

        $paymentRequestStruct = $this->getTestPaymentRequestStruct();
        $this->sofortPaymentRequestService->sendPaymentRequest($paymentRequestStruct);
    }

    public function testIsPaymentSessionValid(): void
    {
        static::assertTrue($this->sofortPaymentRequestService->isPaymentSessionValid());
    }
